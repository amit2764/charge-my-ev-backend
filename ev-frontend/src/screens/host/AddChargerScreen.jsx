...
export default function AddChargerScreen({ onCancel, onCreate }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(e.response?.data?.error || e.message || 'Failed to create charger.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-3 rounded-xl border border-gray-800 bg-gray-900/50 p-4">
      <h3 className="text-base font-bold text-white">Add Charger</h3>
      ...
      <div className="flex gap-2">
        ...
        <button onClick={submit} disabled={saving} className="flex-1 rounded-md bg-cyan-500 px-3 py-2 text-xs font-semibold text-black hover:bg-cyan-400 disabled:opacity-60">{saving ? 'Saving...' : 'Save Charger'}</button>
      </div>
    </div>
  );
}
